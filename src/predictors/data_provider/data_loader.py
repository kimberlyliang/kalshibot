...
def _pick_time_column(df: pd.DataFrame) -> str:
    for candidate in ('date', 'open_time', 'timestamp', 'time', 'datetime'):
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        '''
        # updated to pick a time column, then use that instead
        '''
        original was:
        ...
        if self.features == 'M' or self.features == 'MS':
            #cols_data = df_raw.columns[1:]
            cols_data = [
                col for col in df_raw.columns[1:]
        ...
        df_stamp['date'] = pd.to_datetime(df_stamp.date)
        '''
        df_stamp = df_raw[[time_col]][border1:border2].rename(columns={time_col: 'date'})
        df_stamp['date'] = pd.to_datetime(df_stamp['date'])
        if self.timeenc == 0:
            df_stamp['month'] = df_stamp.date.apply(lambda row: row.month, 1)
            df_stamp['day'] = df_stamp.date.apply(lambda row: row.day, 1)
...
class Dataset_Pred(Dataset):
    ...
    def __read_data__(self):
        ...
        tmp_stamp = df_raw[[time_col]][border1:border2].rename(columns={time_col: 'date'})
